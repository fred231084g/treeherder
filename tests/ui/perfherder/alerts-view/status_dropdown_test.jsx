...
test('Alert is created on Monday, Triage to countdown shows Days left', async () => {
  ...
  expect(dueDateStatusText.slice(0, 9)).toBe('Days left');
});

test('Showing Hours left status when Due date day is the same as current day and alert is created on Wednesday', async () => {
  const alert = testAlertSummaries[0];
